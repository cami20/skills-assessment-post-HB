...
SELECT models.name, brands.name, brands.headquarters FROM models, brands WHERE brands.name = 'Ford' AND models.name = 'Mustang';
...
SELECT * FROM models WHERE model_id > 14 AND model_id < 25;
...
SELECT b.name, m.year, m.name FROM brands AS b LEFT JOIN models AS m on b.brand_id = m.brand_id WHERE m.brand_id IS NOT NULL ORDER BY b.name;
...
SELECT brands.name, brands.founded FROM brands LEFT JOIN models ON brands.brand_id=models.brand_id WHERE models.name IS NULL;
...
SELECT brand_id FROM models GROUP BY brand_id HAVING COUNT(brand_id) > 5;
...
SELECT models.name, models.year FROM brands, models WHERE brands.founded=models.year;